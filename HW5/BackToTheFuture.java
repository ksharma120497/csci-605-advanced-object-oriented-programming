...
class Picture{
    ...
    public void removeHeadOfHangman(int index){
        this.hashInHangman[index] = "0000000";
    }
 ...
 class Game {
    ...
    public static void playTheWordGame(int numberOfTries, Scanner scanner, Dictionary dictionary, Picture picture){
        int wrongGuessCount = 0;
        int wordCounter = 0;
        String answerString=dictionary.getWord();
        int i;
        String[] userProvidedOutputString = dictionary.createStartingString(answerString);
        for ( i = 0; i <= numberOfTries; i++) {
            System.out.println("Wrong guesses: " + wrongGuessCount + " -- Word to guess: " + dictionary.printOutputOfTheGameFromArray(userProvidedOutputString));
            ...
            if (answerString.contains(typedLetter)) {
                ...
                for (int j = 0; j <wrongGuessCount; j++)
                    picture.removeHeadOfHangman(j);
                picture.drawHangman(wordCounter);
            }
            //If the answer is correct it should print the answer and come out of the loop
            if (!dictionary.printOutputOfTheGameFromArray(userProvidedOutputString).contains(".")) {
                System.out.println("Good Job! The word was " + dictionary.printOutputOfTheGameFromArray(userProvidedOutputString));
                break;
            }
        }
  ...
  class Player {
    ...
    public void playerControls(){
        ...
        do{
            picture.initializeHangman();
            dictionary.readWordsFromFile("/Users/kapilsharma/School Work/AOOP HW/src/HW3/hangman_guess_words");
            picture.drawHangman(0);
            game.playTheWordGame(numberOfTries, scanner, dictionary, picture);
